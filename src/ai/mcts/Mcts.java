...
public class Mcts implements AI {
	...
	private void backupNegaMultiMax(MctsNode node, double delta) {
		node.visits++;
		if (node.p1)
			node.value += delta;
		else
		...
		for (final MctsNode parent : node.parents)
			backupNegaMultiMax(parent, delta);
	}

	/*
	 * private void backupNegamax(MctsNode node, double delta) { while (node !=
	 * null) { node.visits++; if (node.p1) node.value += delta; else node.value
	 * -= delta; node = node.parent; } }
	 */
