...
public class MaterialBalanceEvaluator implements IStateEvaluator {
	...
	private double matDif(GameState state, boolean p1) {
					...
					if (state.units[x][y].p1Owner) {
						if (state.units[x][y].unitClass.card == Card.CRYSTAL)
							p1Crystals += (double) state.units[x][y].hp
									/ (double) (state.units[x][y].unitClass.maxHP * state.map.p1Crystals
											.size());
						else {
							p1Units += values
									.get(state.units[x][y].unitClass.card);
							for (final Card card : state.units[x][y].equipment)
								p1Units += values.get(card);
						}
					} else if (state.units[x][y].unitClass.card == Card.CRYSTAL)
		...
		// double p1Val = Math.min(p1Units, p1Crystals) + Math.max(p1Units,
		// p1Crystals) * (Math.min(p1Units, p1Crystals) / MAX_VAL);
		// double p2Val = Math.min(p2Units, p2Crystals) + Math.max(p2Units,
		// p2Crystals) * (Math.min(p2Units, p2Crystals) / MAX_VAL);
		final double p1Val = Math.min(p1Units, p1Crystals)
				+ Math.max(p1Units, p1Crystals);
		final double p2Val = Math.min(p2Units, p2Crystals)
				+ Math.max(p2Units, p2Crystals);
